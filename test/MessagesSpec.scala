...
class MessagesSpec extends GuiceAppSpecBase {
  ...
  "English file" should {
    ...
    "contain the correct messages for the Change Link Contexts (to support Accessibility Software)" in {
      ...
      Messages("worker.optimised.moveWorker.changeLinkContext") mustBe "if your client has the right to move you from the task you originally agreed to do"
      Messages("hirer.optimised.moveWorker.changeLinkContext") mustBe "if your organisation has the right to move the worker from the task they originally agreed to do"
      Messages("worker.optimised.howWorkIsDone.changeLinkContext") mustBe "if your client has the right to decide how the work is done"
      Messages("hirer.optimised.howWorkIsDone.changeLinkContext") mustBe "if your organisation has the right to decide how the work is done"
      Messages("worker.optimised.scheduleOfWorkingHours.changeLinkContext") mustBe "if your client has the right to decide your working hours"
      Messages("hirer.optimised.scheduleOfWorkingHours.changeLinkContext") mustBe "if your organisation has the right to decide the worker’s working hours"
      Messages("worker.optimised.chooseWhereWork.changeLinkContext") mustBe "if your client has the right to decide where you do the work"
      Messages("hirer.optimised.chooseWhereWork.changeLinkContext") mustBe "if your organisation has the right to decide where the worker does the work"
