...
import pages.sections.personalService.{NeededToPayHelperPage, RejectSubstitutePage}
...
class RejectSubstituteControllerSpec extends ControllerSpecBase with MockDataCacheConnector {
  ...
  "RejectSubstitute Controller" must {
    ...
    "redirect to the next page when valid data is submitted" in {
      val validCacheMap = CacheMap(cacheMapId, Map(RejectSubstitutePage.toString -> Json.toJson(Answers("",0))))
