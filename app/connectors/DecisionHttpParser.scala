      ...
      response.status match {
        case Status.OK => {
          ...
          response.json.validate[DecisionResponse].fold(
            invalid => {
              Logger.debug(s"Received Invalid Json from decision connector - $invalid")
              Left(ErrorResponse(Status.INTERNAL_SERVER_ERROR, "Invalid Json received from decision connector"))
            },
            decisionResponse => Right(decisionResponse)
          )
        }
        case status =>
          Logger.warn(s"Unexpected response from decide API - Status $status Response: ${response.body}")
