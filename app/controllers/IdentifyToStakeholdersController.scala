...
class IdentifyToStakeholdersController @Inject()(dataCacheConnector: DataCacheConnector,
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async { implicit request =>
    form.bindFromRequest().fold(
      ...
      value => {
        val answers = compareAndConstructAnswer(request,value,IdentifyToStakeholdersPage)
        dataCacheConnector.save(answers.cacheMap).flatMap(
          _ => {
            val continue = navigator.nextPage(IdentifyToStakeholdersPage, mode)(answers)
            val exit = continue
            decisionService.decide(answers, continue, exit, ErrorTemplate("identifyToStakeholders.title"))
          }
        )
      }
    )
