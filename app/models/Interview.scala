...
object Interview extends JsonObjectSugar {
  ...
  def apply(userAnswers: UserAnswers)(implicit appConfig: FrontendAppConfig): Interview =
    Interview(
      ...
      workerProvidedMaterials = userAnswers.get(CannotClaimAsExpensePage).map(result => result.answer.exists(_ == WorkerProvidedMaterials)),
      workerProvidedEquipment = userAnswers.get(CannotClaimAsExpensePage).map(result => result.answer.exists(_ == WorkerProvidedEquipment)),
      workerUsedVehicle = userAnswers.get(CannotClaimAsExpensePage).map(result => result.answer.exists(_ == WorkerUsedVehicle)),
      workerHadOtherExpenses = userAnswers.get(CannotClaimAsExpensePage).map(result => result.answer.exists(_ == WorkerHadOtherExpenses)),
      expensesAreNotRelevantForRole = userAnswers.get(CannotClaimAsExpensePage).map(result => result.answer.exists(_ == ExpensesAreNotRelevantForRole)),
