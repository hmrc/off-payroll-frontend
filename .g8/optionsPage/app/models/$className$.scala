...
object $className$ {
  ...
  case object $option4key;format="Camel"$ extends WithName("$option4key;format="decap"$") with $className$

  val values: Seq[$className$] = Seq(
    $option1key;format="Camel"$, $option2key;format="Camel"$, $option3key;format="Camel"$, $option4key;format="Camel"$
  )
  ...
  implicit object $className$Reads extends Reads[$className$] {
    override def reads(json: JsValue): JsResult[$className$] = json match {
      ...
      case JsString($option3key;format="Camel"$.toString) => JsSuccess($option3key;format="Camel"$)
      case JsString($option4key;format="Camel"$.toString) => JsSuccess($option4key;format="Camel"$)
